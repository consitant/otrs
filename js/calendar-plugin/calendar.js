(function ($) {
  ...
  /* Garcia: opt array of selected Dates (Array of Date Obj) */  
  var selectedDates;  
  ...
  /* Garcia: Lokalisierung von Monatsnamen und Tagesnamen */
  initCalendar( monthMap,dayMap,alternateDayMap );
    ...
    monthData.forEach(function (week, weekNo) {
      ...
      week.forEach(function (day, dayNo) {
        ...
        if (selectedDate) {
          ...
          /*
          alert("selectedDate.toString(): " + selectedDate.toString() );
          alert("day: " + day );
              */
          /*
           * selectedDate.toString(): Tue May 04 2021 00:00:00 GMT+0200 (Mitteleuropäische Sommerzeit)
           */
        ...
        if (selectedDates && "" == selected ) {
         ...
         /* alert("selectedDates: " + jstr( selectedDates ) )  ; */
            
          if ( selectedDates.includes( day.toString() ) ){ selected = true; };
          selected = selected ? " selected" : "";
        } 
        ...
        /* Garcia: Tag im isoFormat einsetzen */
        var dayIso = moment( day ).format("YYYY-MM-DD");
  ...
  $.fn.calendar = function (options) {
    ...
    /* Garcia: Option für array von selected dates  */
    if (settings.selectedDates ) {
        
      selectedDates = [];
        
      for( var dx in settings.selectedDates ){
          var dVal = settings.selectedDates[ dx ];
          
          if (typeof dVal === "string") {
            dVal = new Date( dVal );
          } 
          dVal.setHours(0, 0, 0, 0);
          selectedDates.push( dVal.toString() );
      }
    }
    ...
    el.off("click", ".day").on("click", ".day", function (e) {
      ...
      settings.onClickDate(date, this);
    });
